    ...
    public class TripAlgo
    {
        ...
        private List<T> FilterAndSort<T>(List<T> list, IsraelRegion area, int budget, DateTime tripDate, TimeOnly startTime, TimeOnly endTime) where T : Place
        {
            return list
                ...
                .Where(a => IsPlaceOpenDuringVisit(a, tripDate, startTime, endTime))
                .OrderByDescending(a => a.Rating) 
                .ThenByDescending(a => a.ReviewsCount) 
                .ToList();
        }
        private bool IsPlaceOpenDuringVisit(Place place, DateTime tripDate, TimeOnly startTime, TimeOnly endTime)
        {
            // מציאת רשומות של שעות פתיחה עבור היום הנתון
            var openingHours = place.OpeningHours
                .FirstOrDefault(oh => oh.Day.Equals(tripDate.DayOfWeek));

            if (openingHours != null)
            {
                TimeOnly openingTime = TimeOnly.Parse(openingHours.OpeningTime);
                TimeOnly closingTime = TimeOnly.Parse(openingHours.ClosingTime);

                // בדיקה האם שעות הביקור נופלות בטווח שעות הפתיחה
                return startTime >= openingTime && endTime <= closingTime;
            }

            return false; // אם לא נמצאו שעות פתיחה עבור היום הנתון
        }
        ...
        public List<Place> SelectBestCombination(Trip trip)
        {
            ...
            // מיון המקומות לפי דירוג ומספר מדרגים
        ...
        private double PriceDropImpact(Place a, List<Place> allPlaces)
        {
            ...
            if (index == -1 || index == allPlaces.Count - 1)
                return 0; // אם המקום הוא האחרון, אין השוואה
